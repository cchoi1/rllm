...
#SBATCH --job-name="deepcoder_1.5b_cm_mc_small_2_steps_sphinx2"
#SBATCH --output=deepcoder_1.5b_cm_mc_small_2_steps_sphinx2.log
...
srun --ntasks=1 --gpus-per-task=1 --exclusive -c 16 --mem=128GB \
...
srun --ntasks=1 --gpus-per-task=${NUM_GPUS} --exclusive -c 104 --mem=896GB \
  ...
  bash -lc "python3 -m examples.context_manager.train_cm \
    ...
    trainer.experiment_name='cm-deepcoder1.5b-mc-8k-2steps' \
