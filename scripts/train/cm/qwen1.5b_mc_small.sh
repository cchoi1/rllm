...
#SBATCH --job-name="cm_mc_small_2_steps"
#SBATCH --output=cm_mc_small_2_steps.log
...
srun --ntasks=1 --gpus-per-task=1 --exclusive -c 8 --mem=64GB \
...
srun --ntasks=1 --gpus-per-task=${NUM_GPUS} --exclusive -c 16 --mem=192GB \
  --output="./cm_mc_small_2steps.log" --error="./cm_mc_small_2steps.log" \
  bash -lc "python3 -m examples.context_manager.train_cm \
    ...
    trainer.experiment_name='cm-qwen1.5b-coder-mc-4k-2steps' \
