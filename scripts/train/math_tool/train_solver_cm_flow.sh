...
#SBATCH --job-name="train_math_tool_train_solver_cm"
#SBATCH --output=train_math_tool_train_solver_cm.log
...
python3 -m examples.math_tool.train_solver_cm_flow \
    ...
    trainer.experiment_name='4b-math-solver-cm' \
...
LOGDIR=$(ls -td /tmp/$USER/ray_*/session_*/logs 2>/dev/null | head -n1)
if [ -n "$LOGDIR" ]; then
