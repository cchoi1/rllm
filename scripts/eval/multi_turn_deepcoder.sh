...
#SBATCH --job-name="eval_cm_multi-turn_sol_multi-turn"
#SBATCH --output=cm_multi-turn_sol_multi-turn.log
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=cchoi1@stanford.edu
#SBATCH --exclude=jagupard[19-20,26-31]

# Start first vLLM server
CUDA_VISIBLE_DEVICES=0 vllm serve /nlp/scr/cchoi1/rllm/checkpoints/rllm-deepcoder/multi-deepcoder1.5b-broadcast-4steps/global_step_55/actor/checkpoint \
  --host 0.0.0.0 --port 12345 \
  --served-model-name agentica-org/DeepCoder-1.5B-Preview \
  --tensor-parallel-size 1 --dtype bfloat16 &

# Start second vLLM server
CUDA_VISIBLE_DEVICES=1 vllm serve /nlp/scr/cchoi1/rllm/checkpoints/rllm-deepcoder/multi-deepcoder1.5b-broadcast-4steps/global_step_55/actor/checkpoint \
...
python -m lcb_runner.multiround.cli \
  ...
  --config ./lcb_runner/config/deepcoder-1.5b-qwen_cm.yaml \
