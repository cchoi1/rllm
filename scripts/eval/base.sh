...
#SBATCH --output=mc_11-2_step50_dual.log
...
# ---------------------------------------------------------------------
# Start server on GPU 0, port 30000
# ---------------------------------------------------------------------
echo "Starting vLLM on GPU 0, port ${PORT0}..."
...
# ---------------------------------------------------------------------
# Start server on GPU 1, port 12345
# ---------------------------------------------------------------------
echo "Starting vLLM on GPU 1, port ${PORT1}..."
...
if [[ "$READY0" -ne 1 ]]; then
  echo "ERROR: vLLM (GPU0) did not start."
  tail -200 "$LOG0" 2>/dev/null || true
  exit 1
fi
...
if [[ "$READY1" -ne 1 ]]; then
  echo "ERROR: vLLM (GPU1) did not start."
  tail -200 "$LOG1" 2>/dev/null || true
  exit 1
fi
...
# (Optional) test both
# ---------------------------------------------------------------------
echo "Testing vLLM on GPU0 (port ${PORT0})..."
curl -fsS -H "Content-Type: application/json" \
  -d "{\"model\":\"$MODEL_NAME\",\"messages\":[{\"role\":\"user\",\"content\":\"test gpu0\"}]}" \
  "${BASE_URL0}/v1/chat/completions" | head -40 || echo "GPU0 test failed; see $LOG0"

echo "Testing vLLM on GPU1 (port ${PORT1})..."
curl -fsS -H "Content-Type: application/json" \
  -d "{\"model\":\"$MODEL_NAME\",\"messages\":[{\"role\":\"user\",\"content\":\"test gpu1\"}]}" \
  "${BASE_URL1}/v1/chat/completions" | head -40 || echo "GPU1 test failed; see $LOG1"

# ---------------------------------------------------------------------
# Run your actual eval here
# You can export both URLs so Python can choose
# ---------------------------------------------------------------------
