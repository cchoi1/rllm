...
# --- use node-local scratch; isolate by user+pid or Slurm job id
...
# # optional: give your own namespace
# export RAY_NAMESPACE="cm-${USER}-$$"
...
python3 -m examples.context_manager.train_cm \
    ...
    +trainer.runtime_env.env_vars.VLLM_USE_V1=1 \
    +trainer.runtime_env.env_vars.VLLM_ALLOW_LONG_MAX_MODEL_LEN=1 \
    +trainer.runtime_env.env_vars.VLLM_ENGINE_ITERATION_TIMEOUT_S=1000000000 \
    +trainer.save_dir='$RUN_DIR/checkpoints' \
    algorithm.adv_estimator=grpo \
    data.train_batch_size=256 \
    data.val_batch_size=512 \
    data.max_prompt_length=27000 \
