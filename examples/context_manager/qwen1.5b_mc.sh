...
python3 -m examples.context_manager.train_cm \
    ...
    trainer.experiment_name="cm-qwen1.5b-coder-mc-rm-test" \
    ...
    +reward_manager.name=rm \
    +reward_manager.world_size=2 \
    +rm.rm_worker_cls=rllm.rewards.cm_rm.CMSolverRMWorker \
    +rm.solver.model="Qwen/Qwen2.5-Coder-1.5B-Instruct" \
    +rm.solver.base_url="http://localhost:12345/v1" \
    +rm.solver.api_key=null \
    +rm.solver.temperature=0.2 \
    +rm.solver.max_tokens=4096