...
def print_prompts_and_responses(results, print_turns: bool = False):
    """Print solver and context manager prompts and responses at each turn.
    
    Args:
        results: List of Episode objects from the workflow execution
        print_turns: If True, print detailed prompts and responses for each turn.
                     If False, only print episode summary (problem, result, metrics).
    """
    for episode_idx, episode in enumerate(results):
        ...
        # Track turns: Initial solver (turn 0), then CM + Solver pairs for each turn
        turn = 0
        solver_turn = 0
        for traj_idx, trajectory in enumerate(episode.trajectories):
            ...
            elif trajectory.name == "context_manager":
                turn += 1  # Increment turn for each CM (starts at 1)
                if print_turns:
                    print(f"\n{'='*80}")
                    print(f"CONTEXT MANAGER - Turn {turn}")
                    print("="*80)
...
if __name__ == "__main__":
    ...
    print_turns = False  # If True, print detailed prompts and responses for each turn

    # Model configuration - both can use the same model or different models
    solver_model_name = "agentica-org/DeepCoder-1.5B-Preview"
    cm_model_name = "agentica-org/DeepCoder-1.5B-Preview"  # CM uses trainable engine
    ...
    engine = AgentWorkflowEngine(
        workflow_cls=DeepCoderCMWorkflow,
        workflow_args={
    ...
    if print_turns:
        ...
        print("="*80)
    print_prompts_and_responses(results, print_turns=print_turns)
    ...
    output_file = f"logs/solver_cm_flow_lcb{split}_{len(tasks)}.json"
