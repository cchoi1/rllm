...
class SolverContextManagerJointWorkflow(Workflow):
    """
    Iterative workflow:
    ...
    async def run(self, task: dict, uid: str, **kwargs) -> Episode:
        ...
        for round_idx in range(1, self.n_rounds + 1):
            ...
            # --- credit the PREVIOUS CM (it produced feedback for THIS round)
            if prev_cm_traj is not None:
                # improvement-based reward (clip if you like)
                cm_impr = round_avg_reward - (prev_round_avg if prev_round_avg is not None else 0.0)
                prev_cm_traj.steps[0].reward = float(cm_impr)   # CM gets RL signal here

                # optional: also give sparse success bonus
                if round_has_correct:
                    prev_cm_traj.steps[0].reward += 1.0

            # mark post-CM correctness
            if round_idx >= 2 and round_has_correct:
                post_cm_any_correct = True

            # --- if there's another round, ask CM for feedback for the NEXT round
            if round_idx < self.n_rounds:
                ...
                # store for next round's back-fill
